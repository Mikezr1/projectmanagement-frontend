// /* eslint-disable @typescript-eslint/no-explicit-any */
// import axios from "axios";
// import { API_BASE_USER } from "../components/constants";

// interface UserDto {
//     // Define properties of UserDto as needed
// }

// const createUser = async (dto: UserDto): Promise<any> => {
//     const response = await axios.post(`${API_BASE_USER}`, dto);
//     return response.data;
// };

// const getUserById = async (id: string): Promise<any> => {
//     if (!id) {
//         throw new Error("error id doesn't exist");
//     }
//     const response = await axios.get(`${API_BASE_USER}/${id}`);
//     return response.data;
// };

// const getAllUsers = async (): Promise<any> => {
//     const response = await axios.get(`${API_BASE_USER}`);
//     return response.data;
// };

// const updateUser = async (id: string, dto: UserDto): Promise<any> => {
//     const response = await axios.put(`${API_BASE_USER}/${id}`, dto);
//     return response.data;
// };

// const deleteUser = async (id: string): Promise<any> => {
//     const response = await axios.delete(`${API_BASE_USER}/${id}`);
//     return response.data;
// };

// const loginUser = async (dto: UserDto): Promise<unknown> => {
//     const response = await axios.post(`${API_BASE_USER}/login`, dto);
//     return response.data;
// };

// const resetPassword = async (dto: UserDto): Promise<any> => {
//     const response = await axios.post(`${API_BASE_USER}/reset-password`, dto);
//     return response.data;
// };

// export default {
//     createUser,
//     getUserById,
//     getAllUsers,
//     updateUser,
//     deleteUser,
//     loginUser,
//     resetPassword,
// }