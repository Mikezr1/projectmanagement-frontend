...
const AddMemberPage = ({ projectId}: AddMemberPageProps) => {
    ...
    const { data: existingMemberIds = [], isLoading: isProjectLoading, isError: isProjectError, error: projectError } = useQuery({
        queryKey: ["projectMembers", projectId],
        queryFn: () => projectService.getUserIdsByProjectId(projectId),
        enabled: !!user?.id
    });
    ...
    if ( isUsersLoading || isProjectLoading) return <p>Loading...</p>;
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div>
                <label htmlFor="developers"><b>Developers:</b></label>
                <select
                    id="developers"
                    name="select"
                    multiple style={{ height: '160px', width: '100%' }}
                    ...
                    >
                        {developers.map(dev => (
                            <option
                                key={dev.id}
                                value={dev.id}
                                disabled={existingMemberIds.includes(dev.id)}
                                >
                    ...
                    onChange={(e) => setSelectedCustomer(e.target.value === "" ? null : Number(e.target.value)
                        ...
                        {customers.map(c => (
                            <option
                                key={c.id}
                                value={c.id}
                                disabled={existingMemberIds.includes(c.id)}
                                >
